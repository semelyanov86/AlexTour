...
class Vtiger_Portal_Connector extends Vtiger_PortalBase_Connector {
    ...
    public function fetchBlocks($id, $module, $parentId = '') {
        $username = Portal_Session::get('username');
        $password = Portal_Session::get('password');
        $this->auth = array('Authorization' => 'Basic '.base64_encode($username.':'.$password));

        $params = array(
            '_operation' => 'FetchBlocks',
            'module' => $module,
            'recordId' => $id,
            'parentId' => $parentId,
            'username' => $username,
    ...
    public function searchAutocomplete($parent, $child, $searchKey) {
        $username = Portal_Session::get('username');
        $password = Portal_Session::get('password');
        $this->auth = array('Authorization' => 'Basic '.base64_encode($username.':'.$password));

        $params = array(
            '_operation' => 'SearchAutocomplete',
            'searchKey' => $searchKey,
            'parent' => $parent,
            'child' => $child,
            'username' => $username,
