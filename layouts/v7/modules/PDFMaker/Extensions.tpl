            ...
            <table class="table table-bordered table-striped">
                <thead>
                    <tr class="blockHeader">
                        <th colspan="2"><div class="textAlignLeft">{vtranslate($arr.label, 'PDFMaker')}
                            {if $arr.download neq ""}<span class="pull-right"><a class="btn" href="{$arr.download}">{vtranslate('LBL_DOWNLOAD', 'PDFMaker')}</a></span>{/if}
                            {if $arr.install neq ""}
                                <span class="pull-right"><button type="button" id="install_{$extname}_btn" class="btn ext_btn btn-success" data-extname="{$extname}" data-url="{$arr.install}">{vtranslate('LBL_INSTALL_BUTTON', 'Install')}</button></span>
                            {/if}
                            {if $arr.button neq ""}
                                <span class="pull-right"><button type="button" id="{$arr.button.type}_{$extname}_btn" class="btn {$extname}_btn {$arr.button.style} ext_btn" data-extname="{$extname}" data-url="{$arr.button.url}">{vtranslate({$arr.button.label}, 'PDFMaker')}</button></span>
                            {/if}</div>
                        </th>
                    </tr>
                </thead>
