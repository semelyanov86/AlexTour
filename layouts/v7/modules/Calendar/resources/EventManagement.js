...
Vtiger_Index_Js("Vtiger_EventManagement_Js",{},{

	overlayContainer : false,
	getOverlayContainer : function(){
		if(this.overlayContainer === false){
			this.overlayContainer = jQuery('#eventsManagementContainer');
		}
	...
	registerStatusCheckboxEvent : function(){
		...
		overlay.on('click','.statusCheckbox',function(e){
			...
			var activitytype = task.data('type');
			var fieldNameValueMap = {'activitytype': activitytype};
			if (element.is(':checked')) {
				element.disable();
				fieldNameValueMap['value'] = 'Held';
				fieldNameValueMap['field'] = 'eventstatus';
				app.helper.showProgress();
				console.log(fieldNameValueMap, task);
	...
	getAllContents : function(params){
		var aDeferred = jQuery.Deferred();
		this.filterRecords(params, "getAllContentsEvents").then(function(data){
	...
	registerDateFilters : function(){
		...
		overlay.on("click",".dateFilters button",function(e){
			...
			jQuery('#eventsManagementContainer .dateFilters button').removeClass('active');
	...
	registerQuickEditTaskEvent : function(){
		var thisInstance = this;
		var overlay = this.getOverlayContainer();
		jQuery('#eventsManagementContainer').on('click', ".quickTask",function(e){
			e.stopImmediatePropagation();
			var target = jQuery(e.currentTarget);
			var quickCreateNode = jQuery('#quickCreateModules').find('[data-name="Events"]');
			...
			app.event.one("post.QuickCreateForm.show",function(event,form){
				...
				if(typeof basicInfo != "undefined"){
					...
					var taskStatus = form.find('select[name="eventstatus"]');
					taskStatus.val('Planned');
					vtUtils.showSelect2ElementView(taskStatus);
					var eventType = form.find('select[name="activitytype"]');
					eventType.val('Other');
					vtUtils.showSelect2ElementView(eventType);
				}
	...
	registerPostQuickCreateSaveEvent : function() {
		...
		app.event.on('post.QuickCreateForm.save',function(event,data){
			...
			var contentsBlock = jQuery("#eventsManagementContainer").find(".contentsBlock ."+priority.toLowerCase()+"");
			thisInstance.clearExistingCustomScroll();
	...
	registerSubjectFieldEnterEvent : function(){
		...
		overlay.on("keypress",".taskSubject",function(e){
			...
			if(keycode == '13'){
				...
				fieldNameValueMap["eventstatus"] = 'Planned';
				fieldNameValueMap["assigned_user_id"] = app.getUserId();
				fieldNameValueMap["activitytype"] = 'Other';
	...
	getAllFilterParams : function(){
		...
		var statusFilter = jQuery('.otherFilters select[name="eventstatus"]').val();
		if(statusFilter){
		...
		overlay.find(".ui-droppable").droppable({
			...
			drop: function( event, ui ) {
				...
				if(draggedElementPriority != priority){
					...
					var activitytype = draggedElementTask.data('type');
					var fieldNameValueMap = {"taskpriority":priority, "activitytype": activitytype};
					app.helper.showProgress();
...
//							app.helper.showVerticalScroll(currentBlock.find("."+priority.toLowerCase()+"-entries"),params);
	...
	loadContent : function(priority){
		var thisInstance = this;
		var blockElement = jQuery("#eventsManagementContainer").find(".contentsBlock."+priority+"");
	...
	initializeTaskStatus : function(){
		...
		var taskStatus = container.find('select[name="eventstatus"]');
		if(taskStatus.length > 0){
			taskStatus.find('[value="Planned"]').attr('selected', "selected");
			taskStatus.find('[value="Held"]').attr('selected', "selected");
			taskStatus.find('[value="Not Held"]').attr('selected', "selected");
			vtUtils.showSelect2ElementView(taskStatus);
			this.loadContents();
		}
	},
	...
	registerEvents : function(){
		...
		vtUtils.registerEventForDateFields(jQuery('#eventsManagementContainer'));
