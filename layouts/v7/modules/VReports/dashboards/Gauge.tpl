...
<div name="panel_content" class="panel_content" style="white-space: nowrap;height: 78%;padding-top: 5px;{if $backgroundColor}background-color:{$backgroundColor}{/if}">
    {foreach from=$DATA item=VALUE key=FIELD_NAME}
        ...
        {if $SYMBOL_PLACEMENT eq 'first'}
            <div class="gauge" style="color:{if $dataColor} {$dataColor} {elseif $HEADER_COLOR} {$HEADER_COLOR} {else} #ffffff; {/if}">{if $icon}<i class="{$icon}"></i>{else}{$USER_CURRENCY_SYMBOL}{/if} {$final_value}</div>
        {else}
            <div class="gauge" style="color:{if $dataColor} {$dataColor} {elseif $HEADER_COLOR} {$HEADER_COLOR} {else} #ffffff; {/if}">{$final_value} {if $icon}<i class="{$icon}"></i>{else}{$USER_CURRENCY_SYMBOL}{/if}</div>
        {/if}
    {/foreach}
    <br>
