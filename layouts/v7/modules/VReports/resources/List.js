 ...
 * All Rights Reserved.
 *************************************************************************************/
Vtiger_List_Js("VReports_List_Js",{

    listInstance : false,

    addReport : function(url){
        var listInstance = VReports_List_Js.getInstance();
        ...
        if(validationResult != true){
            ...
            app.request.post(params).then(function(error,data) {
                ...
                var callBackFunction = function(data){
                    var reportsListInstance = new VReports_List_Js();
    ...
    registerEditFilterClickEvent : function(){
        ...
        listViewFilterBlock.on('mouseup','li i.editFilter',function(event){
            ...
            VReports_List_Js.triggerAddFolder(editUrl);
    ...
    savePinToDashboard : function(parentElement,element, customParams) {
        var listInstance = VReports_List_Js.getInstance();
        var parent = jQuery(parentElement)
        var recordId = parent.data('recordid');
        var type = parent.data('report-type');
        var widgetTitle = 'ChartReportWidget_' + recordId;
        var tabName = element.data('tab-name');
        var tabId = element.data('tab-id');
        var action = 'ChartActions';
        if(type == 'pivot'){
            widgetTitle = 'PivotReportWidget_' + recordId;
            action = 'PivotActions';
        }
        if(type == 'tabular'){
            widgetTitle = 'TabularReportWidget_' + recordId;
        ...
        app.request.post({data: params}).then(function (error,data) {
            if (data.duplicate) {
                var params = {};
                if(type == 'pivot'){
                    params = {message: app.vtranslate('JS_PIVOT_ALREADY_PINNED_TO_DASHBOARD', 'VReports')+' In Tab ' + tabName};
                }
                else{
                    params = {message: app.vtranslate('JS_CHART_ALREADY_PINNED_TO_DASHBOARD', 'VReports')+' In Tab ' + tabName};
                }
                ...
                var title = '';
                if(type == 'pivot'){
                    message = app.vtranslate('JS_PIVOT_PINNED_TO_DASHBOARD', 'VReports');
                    title = app.vtranslate('JSLBL_UNPIN_PIVOT_FROM_DASHBOARD')
                }
                else{
                    message = app.vtranslate('JS_CHART_PINNED_TO_DASHBOARD', 'VReports');
                    title = app.vtranslate('JSLBL_UNPIN_CHART_FROM_DASHBOARD');
                }
    ...
    unpinFromDashboard : function (parentElement,element,customParams) {
        var thisInstance = this;
        var recordId = jQuery(parentElement).data('recordid');
        var tabName = element.data('tab-name');
        var type = jQuery(parentElement).data('report-type');
        var action = 'ChartActions';
        if(type == 'pivot'){
            action = 'PivotActions';
        }
        ...
        app.request.post({data: params}).then(function (error,data) {
            if(data.unpinned) {
                ...
                var title = '';
                if(type == 'pivot'){
                    message = app.vtranslate('JS_PIVOT_REMOVED_FROM_DASHBOARD', 'VReports') + " In Tab " + tabName;
                    title = app.vtranslate('JSLBL_PIN_PIVOT_TO_DASHBOARD','VReports');
                }else{
                    message = app.vtranslate('JS_CHART_REMOVED_FROM_DASHBOARD', 'VReports')+ " In Tab " + tabName;
                    title = app.vtranslate('JSLBL_PIN_CHART_TO_DASHBOARD','VReports');
                }
                app.helper.showSuccessNotification({message:message});
    ...
    registerEventForPinChartToDashboard: function () {
        ...
        jQuery('html').on('click','.dashBoardTab', function(e){
            ...
            if(pinned){
                thisInstance.savePinToDashboard(parent.find('.pinToDashboard'),element, params);
            }else{
                thisInstance.unpinFromDashboard(parent.find('.pinToDashboard'),element, params);
            }
        })
    },
    ...
    removeToggleAddDashboard : function () {
        $('.main-container').on('click',function(){
            $('#page > .dashBoardTabMenu').remove();
        });
    },
        ...
        $('button#phpiniWarnings').on('click',function(){
            ...
            app.request.get({'data': data}).then(function (err, data) {
                ...
                if (!err) {
                    ...
                    $('a.showError').off('click');
                    $('a.showError').on('click',function () {
                        var ulHide = $(this).parent().find('ul.findMissingLink');
                    ...
                    $('a.fixError').off('click');
                    $('a.fixError').on('click',function () {
                        var element = $(this);
                        var dataFix = element.data('fix');
                        var params = {
                            'module': moduleName,
                            'view': 'ListAjax',
                            'mode': 'fixError',
