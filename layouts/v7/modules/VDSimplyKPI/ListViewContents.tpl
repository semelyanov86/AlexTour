					...
					<tr class="emptyRecordsDiv">
						...
						<td colspan="{$COLSPAN_WIDTH}">
							<div class="emptyRecordsContent">
								...
								{vtranslate('LBL_NOT_FOUND', $MODULE)} {vtranslate($MODULE, $MODULE)}.
								...
								{if $IS_MODULE_EDITABLE}
								    ...
								    {if Users_Privileges_Model::isPermitted($MODULE, 'Import') && $LIST_VIEW_MODEL->isImportEnabled()} 
								        {vtranslate('LBL_OR', $MODULE)} <a style="color:blue" href="#" onclick="return Vtiger_Import_Js.triggerImportAction()"> {vtranslate('LBL_IMPORT', $MODULE)} </a>{vtranslate($MODULE, $MODULE)}{else}{vtranslate($SINGLE_MODULE, $MODULE)}
								    {/if}
								{/if}
							</div>
						</td>
