...
{strip}
	...
	{assign var=SELECTED_PICKLIST_FIELDMODEL value= Vtiger_Field_Model::getInstance('eventstatus', $MODULE_MODEL)}
	...
	{foreach key=RECORDID item=RECORD_MODEL from=$TASKS}
		<div class="entries ui-draggable">
			...
			<div class="task clearfix" data-recordid="{$RECORD_MODEL->get('id')}" data-type="{$RECORD_MODEL->get('activitytype')}" data-priority="{$PRIORITY}" data-basicinfo='{json_encode($RECORD_BASIC_INFO)}' style="border-left:4px solid {$COLORS[$PRIORITY]}">
				{assign var=STATUS value=$RECORD_MODEL->get('eventstatus')}
				<div class="task-status pull-left" data-status="{$STATUS}">
					<input class='statusCheckbox' type="checkbox" name="eventstatus" {if $STATUS eq "Held"} checked disabled {/if}/>
				</div>
				<div class='task-body clearfix'>
					<div class="taskSubject pull-left {if $STATUS eq "Held"} textStrike {/if} textOverflowEllipsis" style='width:70%;'>
						<a class="quickPreview" data-id="{$RECORDID}" title="{$RECORD_MODEL->get('subject')}">{$RECORD_MODEL->get('subject')}</a>
					</div>
					{assign var=SELECTED_PICKLISTFIELD_ALL_VALUES value= Vtiger_Util_Helper::getPickListValues('eventstatus')}
