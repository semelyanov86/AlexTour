...
{strip}
	<div id="eventsManagementContainer" class='fc-overlay-modal modal-content' style="height:100%;">
		<input type="hidden" name="colors" value='{json_encode($COLORS)}'>
		<div class="overlayHeader">
			{assign var=HEADER_TITLE value="EVENT MANAGEMENT"}
		...
		<div class='modal-body overflowYAuto'>
			<div class='datacontent'>
							...
							<div class="{strtolower($PRIORITY)}-content content" data-priority='{$PRIORITY}' style="border-bottom: 1px solid {$COLORS[$PRIORITY]};padding-bottom: 10px">
								{if $USER_PRIVILEGES_MODEL->hasModuleActionPermission($MODULE_MODEL->getId(), 'CreateView')}
									<div class="input-group">
										<input type="text" class="form-control taskSubject {$PRIORITY}" placeholder="{vtranslate('LBL_ADD_EVENT_AND_PRESS_ENTER', $MODULE)}" aria-describedby="basic-addon1" style="width: 99%">
				...
				<div class="editTaskContent hide"> 
					{include file="EventManagementEdit.tpl"|vtemplate_path:$MODULE}
				</div> 
			</div>
		</div>
	</div>
