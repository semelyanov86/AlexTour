 ...
 * All Rights Reserved.
 *************************************************************************************/
jQuery.Class("Control_Layout_Fields_Js",{
},{
    fieldValuesCache: {},
    /*
                                ...
                                if(target_element.length>0){
                                    ...
                                    if (value.option == 'hide') {
                                        ...
                                        if(parent_tr.length>0){
                                            ...
                                            var target_div = target_element.closest('div.summaryViewEntries');
                                            target_div.remove();
                                        }
                                    }
                                    if (value.option == 'read_only') {
    ...
    checkConditionToForm:function(all_condition,any_condition,field_name_changed,record_info,role_id){
        ...
        jQuery.each(all_condition,function(key,value){
            ...
            if(form_element.length == 0 && field_name == 'accountname'){
                form_element = jQuery('#EditView').find('[name="related_to_display"]');
            }
    ...
    //    var return_value = '';
    //    var url = 'index.php?module=ControlLayoutFields&action=ActionAjax&mode=getFieldValue';
    //    jQuery.ajax({
    //        url: url,
...
jQuery(document).ready(function(){
    ...
    setTimeout(function()
    {
        setTimeoutCallFunctionFormChange();
    }, 300);
});

function setTimeoutCallFunctionFormChange() {
    if(typeof controlLayoutFields_HideFields ==='undefined' || controlLayoutFields_HideFields == true) {
        var clfInstance = new Control_Layout_Fields_Js();
        clfInstance.registerEvents();
        var view = app.getViewName();
        var module = app.getModuleName();
        if (view == 'Detail') {
            ...
            if (typeof request_mode === 'undefined')
                request_mode = "full";
...
jQuery( document ).ajaxComplete(function(event, xhr, settings) {
    setTimeout(function()
    {
        setTimeoutCallFunction(event, xhr, settings);
    }, 300);
});
function setTimeoutCallFunction(event, xhr, settings){
    if(typeof controlLayoutFields_HideFields ==='undefined' || controlLayoutFields_HideFields == true){
