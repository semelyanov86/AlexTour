...
class EMAILMaker_Module_Model extends EMAILMaker_EMAILMaker_Model {
    ...
    private $profilesActions = array("EDIT" => "EditView",
                                    "DETAIL" => "DetailView",
                                    "DELETE" => "Delete",
    );
    ...
    */
    ...
    private function setLicenseInfo() {
        ...
        $adb = PearDatabase::getInstance();
        $sql = "SELECT version_type, license_key, license_due_date FROM vtiger_emakertemplates_license";
    ...
    public function searchEmails($searchValue, $moduleName = false) {
        ...
        while ($row = $db->fetchByAssoc($result)) {
            ...
            $emailsResult[vtranslate($row['setype'], $row['setype'])][$row['crmid']][] = array('value' => $row['value'],
                ...
                'name' => decode_html($row['label']),'module' => $row['setype']);
        }
    ...
    public function SetLicenseDueDate($license_due_date) {
        ...
        $adb = PearDatabase::getInstance();
        $adb->pquery("UPDATE vtiger_emakertemplates_license SET license_due_date = ?",array($license_due_date));
    }
}