...
class Potentials_Module_Model extends Vtiger_Module_Model {
	...
	public function getPotentialsCountBySalesStage($owner, $dateFilter) {
        ...
        $result = $db->pquery('SELECT COUNT(*) count, vtiger_potential.sales_stage FROM vtiger_potential '
                            ...
                            . 'AND vtiger_potential.sales_stage IN ('.  generateQuestionMarks($picklistvaluesmap).') '
    ...
    {
        global $adb;
        $query = "select * from vtiger_potential inner join vtiger_crmentity on vtiger_potential.potentialid = vtiger_crmentity.crmid where vtiger_potential.potentialname=? and vtiger_crmentity.deleted = ?";
        $result = $adb->pquery($query, array($name, 0));
        $noofrows = $adb->num_rows($result);
        if($noofrows != 0)
        {
            $id = $adb->query_result($result,0,"potentialid");
            return Vtiger_Record_Model::getInstanceById($id, 'Potentials');
        } else {
            return false;
        }
    }

    public static function isContactExist($kuddennr)
    {
        global $adb;
        $query = "select * from vtiger_contactscf inner join vtiger_crmentity on vtiger_contactscf.contactid = vtiger_crmentity.crmid where vtiger_contactscf.cf_1161=? and vtiger_crmentity.deleted = ?";
        $result = $adb->pquery($query, array($kuddennr, 0));
        $noofrows = $adb->num_rows($result);
        if($noofrows != 0)
        {
    ...
    public static function createRecordFromArray($array, $relModel = false)
    {
        ...
        foreach ($array as $module=>$fields) {
            ...
            if ($relModel && $relModel->getModuleName() === 'Contacts') {
                $recordModel->set('contact_id', $relModel->getId());
            }
