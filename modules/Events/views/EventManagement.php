...
class Events_EventManagement_View extends Vtiger_Index_View {

	function __construct() {
		$this->exposeMethod('showManagementView');
		$this->exposeMethod('showEventsView');
		$this->exposeMethod('getAllContents');
		$this->exposeMethod('getAllContentsEvents');
    ...
    public function showEventsView(Vtiger_Request $request) {
        $moduleName = $request->getModule();
        $moduleModel = Vtiger_Module_Model::getInstance($moduleName);
        $statusField = Vtiger_Field_Model::getInstance('eventstatus', $moduleModel);
        $ownerField = Vtiger_Field_Model::getInstance('assigned_user_id', $moduleModel);
        ...
        $viewer->assign('PRIORITIES', array_keys($priorities));
        $viewer->assign('COLORS', $this->generateColors($request));
        $viewer->view('EventManagement.tpl', $moduleName);
    }
    ...
    public function getAllContentsEvents(Vtiger_Request $request) {
        $moduleName = $request->getModule();
        $module = Vtiger_Module_Model::getInstance($moduleName);
        $field = Vtiger_Field_Model::getInstance('taskpriority', $module);
        $priorities = $field->getPicklistValues();
	...
	public function getContentsOfPriority(Vtiger_Request $request, $fetch = false) {
		...
		return $viewer->view('EventManagementContents.tpl', $moduleName, $fetch);
	}
			...
			if ($name == 'date') {
				switch ($value) {
					case 'today':	$conditions[$name] = array();
									...
									$conditions[$name]['fieldName'] = 'date_start';
					...
					case 'all' :	$name = 'status';
									...
									$conditions[$name]['fieldName'] = 'eventstatus';
									break;
				}
			} else if ($name == 'status') {
				$conditions[$name] = array();
