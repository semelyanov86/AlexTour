...
class Vtiger_Viewer extends SmartyBC {
    ...
    function checkLayoutLicense($layoutuid){
        ...
        else $layoutInfo = $adb->query_result_rowdata($layoutResult, 0);


        for($i=0; $i<$rowCount; $i++) {
            $license = $adb->query_result_rowdata($result, $i);
            $server_name=preg_replace('#^www\.(.+\.)#i', '$1', $_SERVER['SERVER_NAME']);

            $ds3 = rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, md5(md5("/^\\d*/M".$license['productkey']."YC/^\\d*/".$server_name."MYC")), base64_decode(rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, md5("/^\\d*/".$server_name."MYC"), base64_decode(rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, md5("/^\\d*/"."MYC"), base64_decode($license['activationkey']), MCRYPT_MODE_CBC, md5(md5("/^\\d*/"."MYC"))), "\0")), MCRYPT_MODE_CBC, md5(md5("/^\\d*/".$server_name."MYC"))), "\0")), MCRYPT_MODE_CBC, md5(md5(md5("/^\\d*/M".$license['productkey']."YC/^\\d*/".$server_name."MYC")))), "\0");

            $jca=json_decode($ds3,true);


            /*
            if(isset($jca["next_check"]) && $jca["next_check"] < time()){
                $this->getKeyFile($license['productkey'],$jca['layoutuid'],$layoutInfo['layoutinfo'],"activation",$jca["next_check_timeout"]);
                return "VALID";
            }
            */
            if(isset($jca) && is_array($jca) && isset($jca['messages']) && isset($jca['layoutuid']) && $jca['layoutuid']==$layoutuid){
                ...
                if(isset($layoutCount) && $layoutCount>0){
                    ...
                    $layoutinfo = json_decode(rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, md5(md5("/^\\d*/M".$ekey."YC/^\\d*/".$ekey."MYC")), base64_decode(rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, md5("/^\\d*/".$ekey.$jca['layoutuid']."MYC"), base64_decode(rtrim(mcrypt_decrypt(MCRYPT_RIJNDAEL_256, md5("/^\\d*/"."MYC"), base64_decode($layoutInfo['layoutinfo']), MCRYPT_MODE_CBC, md5(md5("/^\\d*/"."MYC"))), "\0")), MCRYPT_MODE_CBC, md5(md5("/^\\d*/".$ekey.$jca['layoutuid']."MYC"))), "\0")), MCRYPT_MODE_CBC, md5(md5(md5("/^\\d*/M".$ekey."YC/^\\d*/".$ekey."MYC")))), "\0"),true);
